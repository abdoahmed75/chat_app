...
class likerOffer extends StatefulWidget {
  final String? authToken;
  final int?id;
  const likerOffer({Key? key, this.authToken, this.id}) : super(key: key);

  @override
  _likerOfferState createState() => _likerOfferState();
}

class _likerOfferState extends State<likerOffer> {
  ...
  Future<List> fetchLostData() async {
    var response = await http.post(
      Uri.parse('https://project-graduation.000webhostapp.com/api/helper/get-matches-for-provide-job') ,
      headers: {
        "authToken":token!,
      },
      body:
        ...
        child:Form(
          ...
          child: Column(
            children: [
              ...
              Text('الوظائف المشابه',style: TextStyle(
                  fontSize: 20,
              ...
              Padding(
                ...
                child: StreamBuilder(
                    ...
                    builder: (context,AsyncSnapshot<List> snapshot) {
                      if (snapshot.hasData)
                        return snapshot.data!.isEmpty ||snapshot.data==null?Center(child: Text('لا يوجد وظائف متشابه'),):ListView.builder(
                          shrinkWrap: true,
                          reverse: true,
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            return jobList(
                              ...
                              summary:snapshot.data![index]["summary_about_you"],
...
Widget jobList({
  ...
  required String cirtific,summary,attach,
}){
  return   Padding(
