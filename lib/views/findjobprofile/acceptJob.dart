...
class acceptJob extends StatefulWidget {
  ...
  final int?id;
  const acceptJob({Key? key, this.authToken, this.id}) : super(key: key);
...
class _acceptJobState extends State<acceptJob> {
  ...
  Future<List> fetchLostData() async {
    var response = await http.post(
      Uri.parse('https://project-graduation.000webhostapp.com/api/needer/get-all-accept-my-applying-for-provide-jobs')  ,
      headers: {
        "authToken":token!,
      },
    );
