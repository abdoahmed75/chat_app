...
class JobOfferPerson extends StatefulWidget {
  final int?id;
  final String? authToken;
  const JobOfferPerson({Key? key, this.id, this.authToken}) : super(key: key);

  @override
  _JobOfferPersonState createState() => _JobOfferPersonState();
}

class _JobOfferPersonState extends State<JobOfferPerson> {
  ...
  Future<List> fetchLostData() async {
    var response = await http.post(
        Uri.parse('https://project-graduation.000webhostapp.com/api/helper/get-provide-jop-applyers')  ,
        headers: {
          "authToken":token!,
        },
        body:<String, String>{
        ...
        child:Form(
          ...
          child: Column(
            children: [
              ...
              Text('الاشخاص المتقدمين على الوظيفه',style: TextStyle(
                  fontSize: 20,
                        ...
                        return snapshot.data!.isEmpty ||snapshot.data==null?Center(child: Text('لا يوجد متقدمين'),):ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            return jobList(
                              ...
                              address:snapshot.data![index]["user"]["phone"],
                              phone:snapshot.data![index]["user"]["main_address"],
                            );
