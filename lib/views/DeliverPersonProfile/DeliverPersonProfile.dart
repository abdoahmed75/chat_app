...
class DeliverPersonProfile extends StatefulWidget{
  final int?id;
  final String? authToken;
  const DeliverPersonProfile({Key? key, this.id, this.authToken}) : super(key: key);

  @override
  _DeliverPersonProfileState createState() => _DeliverPersonProfileState();
}

class _DeliverPersonProfileState extends State<DeliverPersonProfile> {
  ...
  Future<List> fetchLostData() async {
    var response = await http.post(
        Uri.parse('https://project-graduation.000webhostapp.com/api/helper/get-support-applyers-thing-to-be-done')  ,
        headers: {
          "authToken":token!,
        },
        body:<String, String>{
  ...
  void AccaptePersonW (String id)async
  {
    await http.post(
        Uri.parse('https://project-graduation.000webhostapp.com/api/helper/response-to-applyer-support-thing-to-be-done'),
        headers: {
          "authToken": token!,
        },
        body: <String,String>
        {
          "needer_post_id":'${widget.id!}',
          "response":"yes"
        }
        ...
        child:Form(
          ...
          child: Column(
            children: [
              ...
              Text('الاشخاص طالبى الخدمه',style: TextStyle(
                  fontSize: 20,
