...
class acceptNeed extends StatefulWidget {
  final String? authToken;
  final int?id;
  const acceptNeed({Key? key, this.authToken, this.id}) : super(key: key);

  @override
  _acceptNeedState createState() => _acceptNeedState();
}

class _acceptNeedState extends State<acceptNeed> {
  ...
  Future<List> fetchLostData() async {
    var response = await http.post(
      Uri.parse('https://project-graduation.000webhostapp.com/api/needer/get-accept-applyer-to-help-money-post')  ,
      headers: {
        "authToken":token!,
      },
      body:
        ...
        child:Form(
          ...
          child: Column(
            children: [
              ...
              Text('المقبولين',style: TextStyle(
                  fontSize: 20,
              ...
              Padding(
                ...
                child: StreamBuilder(
                    ...
                    builder: (context,AsyncSnapshot<List> snapshot) {
                      if (snapshot.hasData)
                        return snapshot.data!.isEmpty ||snapshot.data==null?Center(child: Text('لا يوجد مقبولين'),):ListView.builder(
                          shrinkWrap: true,
                          reverse: true,
